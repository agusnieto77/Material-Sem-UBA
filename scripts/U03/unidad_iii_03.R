...
for(url in urls[1:10]){links <- append(links, read_html(url) |> html_elements('.mvp-blog-story-wrap a, .mvp-widget-feat2-left a, .mvp-widget-feat2-right a') |> html_attr('href'))}

# Corremos el for para extraer el contenido de los primeros 9 links escrapeados con el for anterior
for(link in links[1:9]){
  html <- read_html(link)
  fechas    <- append(fechas,    html |> html_elements('.post-date') |>                      html_text2())
  titulares <- append(titulares, html |> html_elements('.mvp-post-title') |>                 html_text2())
  notas     <- append(notas,     html |> html_elements('.theiaPostSlider_preloadedSlide') |> html_text2())
