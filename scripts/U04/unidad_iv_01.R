...
require(spacyr)              # para lematizar y detectar entidades
...
rm(minhash,scores_rep,scores,pares_candidatxs_lsh,notas_id_red,cubo_lsh,corpus_minhash,
   par_notas_red,scores_rep_id,base_lc_notas,scores_rep_join,scores_rep_join_fechas, notas_com)
...
set.seed(890)
(tm_lda_notas <- textmodel_lda(x = dfm_notas, k = 2))
...
(notas_clas_5 <- notas_norm_key_words_lemmas_entities_acc %>% 
    mutate(clase_tm = dfm_notas$topico,
           clase_tm = ifelse(clase_tm == 'topic2', 'conflicto', 'no_conflicto'),
